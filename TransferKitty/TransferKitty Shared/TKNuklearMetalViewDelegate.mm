...
- (void)uniformBuffer:(id<MTLBuffer>)buffer { _uniformBuffer = buffer; }
- (void)vertexBuffer:(id<MTLBuffer>)buffer { _vertexBuffer = buffer; }
- (void)indexBuffer:(id<MTLBuffer>)buffer { _indexBuffer = buffer; }

- (id<MTLBuffer>)uniformBuffer { return _uniformBuffer; }
- (id<MTLBuffer>)vertexBuffer { return _vertexBuffer; }
- (id<MTLBuffer>)indexBuffer { return _indexBuffer; }
...
-(void)setDelegate:(nonnull id<TKNuklearFrameDelegate>)delegate { _frameDelegate = delegate; }
-(id<TKNuklearFrameDelegate>)delegate { return _frameDelegate; }
...
-(nullable instancetype)initWithMetalDevice:(nonnull id<MTLDevice>)device
   ...
   if (!self) { return nil; }
    ...
    for(NSUInteger i = 0; i < FrameCount; i++) {
        ...
        [currentFrame uniformBuffer:[_device newBufferWithLength:sizeof(matrix_float4x4) options:MTLResourceStorageModeShared]];
    ...
    _nuklear._fontPtr = nk_font_atlas_add_from_memory(&_nuklear._fontAtlas, (void*)getFontBytes(), getFontByteLength(), 48, 0);
    ...
    [_fontTexture replaceRegion:MTLRegionMake2D(0, 0, imageWidth, imageHeight)
                  mipmapLevel:0 withBytes:imageBytes bytesPerRow:imageBytesPerRow];
 ...
 commandBuffer:(nonnull id<MTLCommandBuffer>)commandBuffer
  drawableSize:(CGSize)drawableSize {
    ...
    nk_draw_foreach(cmd, currentFrame.contextPtr, &_nuklear._cmdBuffer) {
        ...
        id<MTLTexture> fontTexture = (__bridge id<MTLTexture>)(textureHandle);
...
-(void)setDelegate:(nonnull id<TKNuklearFrameDelegate>)delegate { [_renderer setDelegate:delegate]; }
-(id<TKNuklearFrameDelegate>)delegate { return [_renderer delegate]; }
...
- (void)drawInMTKView:(nonnull MTKView *)view {
    ...
    [_renderer drawNextFrameToRenderPass:view.currentRenderPassDescriptor commandBuffer:commandBuffer drawableSize:view.drawableSize];
