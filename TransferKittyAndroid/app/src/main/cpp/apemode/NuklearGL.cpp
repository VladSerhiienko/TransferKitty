...
NK_API void nk_apemode_render( enum nk_anti_aliasing, int max_vertex_buffer, int max_element_buffer );
...
NK_API void nk_apemode_device_create( void ) {
    ...
    dev->program = NvGLSLProgram::createFromStrings(vertex_shader, fragment_shader, 0);
...
//    assert( status == GL_TRUE );
    ...
    dev->uniform_tex  = glGetUniformLocation( dev->program->getProgram(), "Texture" );
    dev->uniform_proj = glGetUniformLocation( dev->program->getProgram(), "ProjMtx" );
    dev->attrib_pos   = glGetAttribLocation( dev->program->getProgram(), "Position" );
    dev->attrib_uv    = glGetAttribLocation( dev->program->getProgram(), "TexCoord" );
...
NK_INTERN void nk_apemode_device_upload_atlas( const void *image, int width, int height ) {
    struct nk_apemode_device *dev = &nk_apemode_global_state.ogl;
...
NK_API void nk_apemode_render( enum nk_anti_aliasing AA, int max_vertex_buffer, int max_element_buffer ) {
    struct nk_apemode_device *dev = &nk_apemode_global_state.ogl;
...
//    glUseProgram( dev->prog );
    ...
    glViewport( 0, 0, (GLsizei) nk_apemode_global_state.display_width, (GLsizei) nk_apemode_global_state.display_height );
    {
        ...
        elements = nk_apemode_global_state.ogl.buffer_pool.data( ) + max_vertex_buffer;

        /* fill convert configuration */
        struct nk_convert_config config;
        static const struct nk_draw_vertex_layout_element vertex_layout[] = {
            {NK_VERTEX_POSITION, NK_FORMAT_FLOAT, NK_OFFSETOF( struct nk_apemode_vertex, position )},
            {NK_VERTEX_TEXCOORD, NK_FORMAT_FLOAT, NK_OFFSETOF( struct nk_apemode_vertex, uv )},
            {NK_VERTEX_COLOR, NK_FORMAT_R8G8B8A8, NK_OFFSETOF( struct nk_apemode_vertex, col )},
        ...
        nk_convert( &nk_apemode_global_state.ctx, &dev->cmds, &vbuf, &ebuf, &config );

        /* allocate vertex and element buffer */
        glBindBuffer( GL_ARRAY_BUFFER, dev->vbo );
        glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, dev->ebo );
        glBufferData( GL_ARRAY_BUFFER, vbuf.size, vertices, GL_STREAM_DRAW );
        glBufferData( GL_ELEMENT_ARRAY_BUFFER, ebuf.size, elements, GL_STREAM_DRAW );

        glEnableVertexAttribArray( (GLuint) dev->attrib_pos );
        glEnableVertexAttribArray( (GLuint) dev->attrib_uv );
        glEnableVertexAttribArray( (GLuint) dev->attrib_col );

        glVertexAttribPointer( (GLuint) dev->attrib_pos, 2, GL_FLOAT, GL_FALSE, dev->vertex_stride, (void *) dev->offset_pos );
        glVertexAttribPointer( (GLuint) dev->attrib_uv, 2, GL_FLOAT, GL_FALSE, dev->vertex_stride, (void *) dev->offset_uv );
        glVertexAttribPointer( (GLuint) dev->attrib_col, 4, GL_UNSIGNED_BYTE, GL_TRUE, dev->vertex_stride, (void *) dev->offset_col );
        ...
        nk_draw_foreach( cmd, &nk_apemode_global_state.ctx, &dev->cmds ) {
            ...
            glScissor( ( GLint )( cmd->clip_rect.x * nk_apemode_global_state.fb_scale.x ),
                       ( GLint )( ( nk_apemode_global_state.height - ( GLint )( cmd->clip_rect.y + cmd->clip_rect.h ) ) *
...
NK_API void nk_apemode_char_callback( unsigned int codepoint ) {
    if ( nk_apemode_global_state.text_len < NK_GLFW_TEXT_MAX )
        nk_apemode_global_state.text[ nk_apemode_global_state.text_len++ ] = codepoint;
}
...
NK_INTERN void nk_apemode_clipbard_paste( nk_handle usr, struct nk_text_edit *edit ) {
...
NK_INTERN void nk_apemode_clipbard_copy( nk_handle usr, const char *text, int len ) {
    //    char *str = 0;
    //    (void) usr;
    //    if ( !len )
    //        return;
...
NK_API void nk_apemode_font_stash_end( void ) {
    const void *image;
    int         w, h;
    image = nk_font_atlas_bake( &nk_apemode_global_state.atlas, &w, &h, NK_FONT_ATLAS_RGBA32 );
    ...
    if ( nk_apemode_global_state.atlas.default_font )
        nk_style_set_font( &nk_apemode_global_state.ctx, &nk_apemode_global_state.atlas.default_font->handle );
}

NK_API void nk_apemode_new_frame( float width, float height, float display_width, float display_height ) {
    int                i;
    double             x, y;
    ...
    //        glfwSetInputMode( nk_apemode_global_state.win, GLFW_CURSOR, GLFW_CURSOR_HIDDEN );
    //    else if ( ctx->input.mouse.ungrab )
    //        glfwSetInputMode( nk_apemode_global_state.win, GLFW_CURSOR, GLFW_CURSOR_NORMAL );
    //
    ...
    //        glfwSetCursorPos( nk_apemode_global_state.win, ctx->input.mouse.prev.x, ctx->input.mouse.prev.y );
    //        ctx->input.mouse.pos.x = ctx->input.mouse.prev.x;
    //        ctx->input.mouse.pos.y = ctx->input.mouse.prev.y;
    //    }
    //
    //    nk_input_button( ctx, NK_BUTTON_LEFT, (int) x, (int) y, glfwGetMouseButton( win, GLFW_MOUSE_BUTTON_LEFT ) ==
    //    GLFW_PRESS );
    //    nk_input_button(
    //        ctx, NK_BUTTON_MIDDLE, (int) x, (int) y, glfwGetMouseButton( win, GLFW_MOUSE_BUTTON_MIDDLE ) == GLFW_PRESS );
    //    nk_input_button( ctx, NK_BUTTON_RIGHT, (int) x, (int) y, glfwGetMouseButton( win, GLFW_MOUSE_BUTTON_RIGHT ) ==
    //    GLFW_PRESS );
...
void nk_apemode_setup_style() {
    ...
    auto defaultFont = nk_font_atlas_add_from_memory( atlas, (void *) s_droidSansTtf, sizeof( s_droidSansTtf ), 48, 0 );
