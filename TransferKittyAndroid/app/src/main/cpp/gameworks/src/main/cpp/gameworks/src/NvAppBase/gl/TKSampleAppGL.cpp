...
#include "NvAppBase/android/TKAppContextEGL.h"
...
TKSampleAppGL::TKSampleAppGL() : TKSampleApp() {
}

TKSampleAppGL::~TKSampleAppGL() {
}

bool TKSampleAppGL::platformInitRendering(void) {
    ...
    // (since unbroken line-wrap with extensions hurts search)
    {
        ...
        while (tok.readToken()) {
            ...
            if (exbuf.length() + delimLen + ltl >
                lineMaxLen) { // adding would exceed max, print and clear
                if (exbuf.length())
                    LOGI("%s", exbuf.c_str());
                exbuf.clear();
                if (ltl >
                    lineMaxLen) { // alone is too long, print alone and continue
                    LOGI("%s", s);
                }
    ...
    bool hasDXT =
        getGLContext()->isExtensionSupported(
...
void TKSampleAppGL::platformInitUI(void) {
    mContext->initUI();
}

bool TKSampleAppGL::requireExtension(const char* ext, bool exitOnFailure) {
    if (!getGLContext()->isExtensionSupported(ext)) {
        if (exitOnFailure) {
            std::string caption =
                std::string(
                    "The current system does not appear to support the "
                    "extension ") +
                std::string(ext) +
                std::string(
                    ", which is required by the sample.  "
                    "This is likely because the system's GPU or driver does "
...
bool TKSampleAppGL::requireMinAPIVersion(const NvGLAPIVersion& minApi,
                                         bool exitOnFailure) {
...
#endif
            sprintf(caption,
                    "The current system does not appear to support the minimum "
                    "GL API required "
                    "by the sample (requested: %s %d.%d, got: %s %d.%d).  This "
                    "is likely because the system's GPU or driver "
                    "does not support the API.  Please see the sample's source "
                    "code for details",
                    (minApi.api == NvGLAPI::GL) ? "GL" : "GLES",
                    minApi.majVersion,
                    minApi.minVersion,
                    (api.api == NvGLAPI::GL) ? "GL" : "GLES",
                    api.majVersion,
                    api.minVersion);
#ifdef _WIN32
#pragma warning(pop)
...
void TKSampleAppGL::platformLogTestResults(float frameRate, int32_t frames) {
    // writeLogFile(mTestName, true, "GL_VENDOR %s\n", glGetString(GL_VENDOR));
    // writeLogFile(mTestName, true, "GL_RENDERER %s\n",
    // glGetString(GL_RENDERER)); writeLogFile(mTestName, true, "GL_EXTENSIONS
    // %s\n", glGetString(GL_EXTENSIONS)); if (m_testModeIssues !=
    // TEST_MODE_ISSUE_NONE) {
    //     writeLogFile(mTestName, true, "\nWARNING - there were potential test
    //     mode anomalies\n"); if (m_testModeIssues & TEST_MODE_FBO_ISSUE) {
    //         writeLogFile(mTestName, true, "\tThe application appears to have
    //         explicitly bound the onscreen framebuffer\n"
    //             "\tSince the test was being run in offscreen rendering mode,
    //             this could invalidate results\n"
    //             "\tThe application should be checked for glBindFramebuffer of
    //             0\n\n");
    //     }
    // }
}
...
int32_t TKSampleAppGL::getUniqueTypeID() {
    return uniqueTypeID;
}

bool TKSampleAppGL::isType(TKAppBase* app) {
    return app->getUniqueTypeID() == uniqueTypeID;
}

static NvGLConfiguration sDefaultConfig(
    NvGLAPIVersionGL4_4(), 8, 8, 8, 8, 16, 0, 0);

#ifdef ANDROID
static bool localEGLChooser(EGLDisplay disp,
                            EGLint apiSupport,
    ...
    for (i = 0; i < count; i++) {
        ...
        eglGetConfigAttrib(
            disp, configs[i], EGL_RENDERABLE_TYPE, &renderableFlags);
        ...
        // Used when a feature greatly under-represents the requested values,
        // but exists
        ...
        if (sDefaultConfig.depthBits > 0) {
            // If the user requests a depth buffer, then we want to match what
            // they request _or_ better.  Larger will be a minor penalty,
            // smaller is major, none is a showstopper.  Same for stencil.
            if (depthBits == 0)
                continue;
            ...
            if (depthBits < sDefaultConfig.depthBits) {
                ...
                // buffer deeper than we want - penalty point for each extra 8
                // bits
                penalty += MINOR_PENALTY *
                           ((depthBits - sDefaultConfig.depthBits) >> 3);
            }
        } else {
            // If the user requests _no_ depth buffer, then it is a minor
            // penalty to have one, based on size of it
            if (depthBits > 0) {
                // buffer deeper than we want - penalty point for each extra 8
                // bits
        ...
        if (sDefaultConfig.stencilBits > 0) {
            // If the user requests a stencil buffer, then we want to match what
            // they request _or_ better.  Larger will be a minor penalty,
            // smaller is major, none is a showstopper.
            if (stencilBits == 0)
                continue;
            ...
            if (stencilBits < sDefaultConfig.stencilBits) {
                ...
                // bits
                penalty += MINOR_PENALTY *
                           ((stencilBits - sDefaultConfig.stencilBits) >> 3);
            }
        } else {
            // If the user requests _no_ stencil buffer, then it is a minor
            // penalty to have one, based on size of it
            if (stencilBits > 0) {
                // buffer deeper than we want - penalty point for each extra 8
                // bits
        ...
        // MSAA cannot be a complete requirement, so we never filter out.  But
        // asking for it should net _some_ # of samples
        ...
        const int requestedRGB = sDefaultConfig.redBits +
                                 sDefaultConfig.greenBits +
                                 sDefaultConfig.blueBits;
        ...
        LOGI(
            "Config[%d]: R%dG%dB%dA%d D%dS%d MSAA=%d  Type=%04x Render=%04x "
            "(penalties: %d)",
            i,
            redBits,
            greenBits,
    ...
    api = (sDefaultConfig.apiVer.api == NvGLAPI::GL) ? EGL_OPENGL_API
                                                     : EGL_OPENGL_ES_API;
...
static bool hdrConfigChooser(EGLDisplay disp,
                             EGLint apiSupport,
                             EGLConfig& bestConfig,
                             EGLint& api,
                             EGLint& apiMajVer,
    ...
    const EGLint AttributesHDR[] = {EGL_RED_SIZE,
                                    EGLMinRedBits,
                                    EGL_GREEN_SIZE,
                                    EGLMinGreenBits,
                                    EGL_BLUE_SIZE,
...
bool TKSampleAppGL::initialize(const NvPlatformInfo& platform,
                               int32_t width,
                               int32_t height) {
    ...
    NvGLFWContextGL* contextGL =
        new NvGLFWContextGL(sDefaultConfig,
                            NvPlatformCategory::PLAT_DESKTOP,
