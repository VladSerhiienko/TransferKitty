...
static void nv_apqqp_write_cmd(struct android_app* android_app, int8_t cmd) {
    pthread_mutex_lock(&android_app->mutex);
    android_app_write_cmd(android_app, cmd);
    pthread_mutex_unlock(&android_app->mutex);
}
...
Java_vserhiienko_transferkitty_TKMainActivity_postRedraw(JNIEnv *env, jobject instance, jlong time) {
    if (sAndroidApp)
        nv_app_post_redraw(sAndroidApp, time);
}

//JNIEXPORT void JNICALL
//Java_vserhiienko_transferkitty_TKMainActivity_postRedraw(JNIEnv *env, jobject thiz, jlong time) {
//    // TODO: implement postRedraw()
//}